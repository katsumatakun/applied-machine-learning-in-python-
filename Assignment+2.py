...
#         var3=poly.fit_transform(var2)
        var3=poly.transform(var2)
        ary.append(linreg.predict(var3))
    return np.array(ary)
answer_one()
...
def answer_five():
    ...
    x = np.copy(clf.feature_importances_)
    ...
    lst = []
    for i in a:
        lst.append(X_train2.columns[list(clf.feature_importances_).index(i)])
        print(list(clf.feature_importances_).index(i))
    # Your code here
    lst.reverse()
    return lst
